...
# ── OPERATIONAL RISK / RCSA ──────────────────────────────────────
elif page == "📊 Operational Risk — RCSA":
    ...
    with f1:
        process_filter = st.multiselect("Business Process", df_rcsa["Business Process"].unique(), default=list(df_rcsa["Business Process"].unique()))
    with f2:
        residual_filter = st.multiselect("Residual Risk", df_rcsa["Residual Risk"].unique(), default=list(df_rcsa["Residual Risk"].unique()))
    with f3:
        status_filter = st.multiselect("Status", df_rcsa["Status"].unique(), default=list(df_rcsa["Status"].unique()))
